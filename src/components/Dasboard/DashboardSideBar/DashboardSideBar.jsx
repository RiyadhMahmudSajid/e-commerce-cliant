...
import {
  Home, LayoutGrid, PlusCircle, Users, Box, Settings, LogOut, Menu, X, ChevronRight, Folder, LayoutDashboard, ShoppingBag, Heart, Star, UserCircle, MapPin, HeadphonesIcon
} from "lucide-react";
...
const AdminSidebar = () => {
  ...
  const userLinks = [
    ...
    { to: "/dashboard/user/settings", icon: Settings, label: "Settings" },
    ...
    <>
      ...
      <div className="lg:hidden fixed top-4 left-4 z-[40]">
        <button
          ...
          className="p-2 bg-bg-primary   text-text-main"
        >
      ...
      <aside
        className={`fixed top-0 left-0 h-full w-72 bg-bg-primary border-r border-border-color z-50 p-6 flex flex-col transition-transform duration-300
      ...
      >
        ...
        <div className="mb-10 mt-4 lg:mt-0">
          <div className="flex items-center gap-3 px-2 mb-2">
            ...
            <div>
              <p className="text-[10px] font-black text-accent uppercase tracking-[0.2em] leading-tight">
                Admin Panel
              </p>
          ...
          {navLink.map((link) => (
            ...
            >
              {({ isActive }) => (
                <div
                  className={`
                    group flex items-center justify-between px-4 py-3.5 rounded-2xl font-bold text-sm transition-all duration-200
                    ${isActive
                      ? "bg-accent text-white shadow-xl shadow-accent/25 translate-x-1"
        ...
        <div className="mt-auto pt-6 border-t border-border-color">
          <button
            ...
            className="group w-full flex items-center gap-3 px-5 py-4 rounded-2xl font-black text-sm bg-danger/5 text-danger border border-danger/10 hover:bg-danger hover:text-white transition-all shadow-sm hover:shadow-danger/20"
          >
            <LogOut
              size={19}
          ...
          <p className="text-[10px] text-center text-text-muted mt-6 font-bold uppercase tracking-widest opacity-40">
            v2.4.0 • Enterprise
          </p>
        </div>
      </aside>
